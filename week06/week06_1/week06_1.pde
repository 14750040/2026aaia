...
//修改自 week05-2 好玩的程式設計兩層for(迴圈)配合 if.. else if.. else
void setup() {
...
void draw(){
  for (int i=0;i<10; i++) { // 上週是 for y 現在改成 for i 左手i 
    for (int j=0;j<16; j++) { // 上週是 for x 現在改成forj右手j 
      if(a[i][j]==1) fill(#5DFF1F); // 若陣列有值,設綠色 白色
      else fill(255);//沒有值,設白色 
      rect (j*50, i*50, 50, 50);//畫四邊形
    }
  }
/* 很多行的註解 用斜線星這幾行可以不用寫
  for (int y=0; y<500; y+=50){ // 裡面for 迴圈 y 座標
     for (int x=0; x<800; x+=50){ // 裡面for 迴圈 x 座標
